...
describe('adapt', () => {
    it('maps layout and index page', () => {
        ...
        const routes = adapt(tree, {
            resolve: (path: string, type: string) => ({
                path,
                type,
            }),
        });
        ...
        expect((routes[0]?.component as { type: string })?.type).toBe('layout');

        const children = routes[0]?.children ?? [];
        expect(children.some((r: Route) => r.path === '')).toBe(true);
    ...
    it('maps catch-all and optional catch-all segments', () => {
        ...
        expect(shop?.children?.[0]?.path).toBe(':path(.*)+');

        const docs = rootChildren.find((r: Route) => r.path === 'docs');
        expect(docs).toBeDefined();
        expect(docs?.children?.[0]?.path).toBe(':slug(.*)*');
    });
